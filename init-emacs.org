...
org-mode はビルトインのものを利用するように変更した。
...
- :custom, :custom-face :: custom-theme-set-variables, face-defface-spec を設定する
  - custome-face の設定内容の優先度は低いので、:config で custom-set-faces を使うようにする
...
- :hook :: add-hook を行う。 -hook のサフィックスは明示するようにした(use-package-hook-name-suffix nil)。暗黙的に:commandsに追加される

- :config :: パッケージのロード後に行われる。 :config に use-packageをネストさせるのは止める

use-package はビルトインのものを使うようにした。((straight-use-package 'use-package) を削除)
...
#+begin_src emacs-lisp
(defun my/load-my-eld (filename key)
  "指定したmy-data/*.eldファイルからデータを読み込み、keyに対応する値を返します。ファイルがない場合はnilを返します。"
  (when (file-exists-p (locate-user-emacs-file (file-name-concat "my-data" filename)))
    (with-temp-buffer
      (insert-file-contents (locate-user-emacs-file (file-name-concat "my-data" filename)))
      (goto-char (point-min))
...
(setq next-screen-context-lines  5)                             ;1画面スクロール時に残す前頁の行数
(setq scroll-margin 5)                                          ;recenter-top-bottom 時に先頭末尾に残す行数
...
(use-package desktop ;デスクトップの状態を保存する
  ...
  :hook
  (desktop-after-read-hook . (lambda() (setq register-alist nil)))) ; ウィンドウ構成がレジスタに残っていると、レジスタ全般が動かないので起動時にレジスタを消す
#+end_src
...
(use-package solarized-theme
  ...
  (custom-set-faces '(outline-1 ((t (:foreground "SkyBlue3")))))
  (custom-set-faces '(org-level-1 ((t (:foreground "SkyBlue3")))))  ;org-level-1 は outline-1を継承しているっぽいんだけど、outline-1だけでは変わらなかった。
  )
...
(use-package minions
  ...
  :custom
  (minions-prominent-modes '(flymake-mode copilot-mode)))
#+end_src
...
(use-package whitespace
  ...
  :hook
  ((conf-mode-hook prog-mode-hook text-mode-hook) . whitespace-mode)

  :config
  (custom-set-faces '(whitespace-trailing ((t (:background "DarkRed")))))
  (custom-set-faces '(whitespace-empty ((t (:foreground "DarkSlateGrey")))))
  (custom-set-faces '(whitespace-hspace ((t (:foreground "red" :background "DarkRed" :underline t)))))
  (custom-set-faces '(whitespace-newline ((t :foreground "gray30"))))
  (custom-set-faces '(whitespace-tab  ((t (:underline (:style wave)))))))
#+end_src
...
(use-package mozc
  ...
  :bind
  ([remap toggle-input-method] . my/input-method-on)
  ("M-\\" . my/input-method-off)

  :config
  ;; ; mozc.el と mozc_emacs_helper の仕様の差異を埋める (組み合わせによるのでホスト個別の設定に移す)
  ...
  ; かな漢が有効なときにカーソルの色を変更する
  (add-hook 'input-method-activate-hook 'check-mozc-and-set-cursor-color)
  (add-hook 'input-method-deactivate-hook 'check-mozc-and-set-cursor-color)
  (run-at-time t 2
               'check-mozc-and-set-cursor-color))
...
(use-package tab-bar
  ...
  (custom-set-faces '(tab-bar-tab ((t (:background "#073642")))))
  (custom-set-faces '(tab-bar-tab-inactive ((t (:foreground "#586e75" :background "#002b36"))))))
...
(use-package ace-window
  ...
  ("C-x o" . ace-window)
  :hook
  ((prog-mode-hook text-mode-hook) . ace-window-posframe-mode)
...
(use-package tempbuf
  ...
  (help-mode-hook . turn-on-tempbuf-mode)
  ;; (special-mode-hook . turn-on-tempbuf-mode)
  ;; (embark-collect-mode-hook . turn-on-tempbuf-mode)
  ;; (grep-mode-hook . turn-on-tempbuf-mode)
  (emacs-lisp-compilation-mode . turn-on-tempbuf-mode)
  (diary-mode-hook . turn-on-tempbuf-mode)
  (diary-fancy-display-mode-hook . turn-on-tempbuf-mode)
...
| html       | yes          | mhtml-mode, html-mode  | html-ts-mode, mhtml-ts-mode   | remap      |
| java       |              |                        | java-ts-mode                  |            |
...
| ruby       | yes          | ruby-mode              | ruby-ts-mode                  | remap      |
...
[[id:204fa87b-dfcd-443b-a4db-caaf9ab79e4b][Emacs with AI chat]]

** gptel
APIキーはデフォルトでauthinfoから読み込まれる。machine が api.openai.com で、 login が apikey のものが参照される。
他のLLMバックエンドの設定はホスト個別の設定で行う。

- コンテキストの指定
  - gptelはデフォルトでバッファ先頭からカーソル位置までがコンテキストに含まれる
  - gptel-org-branching-context t でサブツリーをコンテキストとすることができる
    - 少し使い勝手が悪いので無効にしてある
  - バッファを狭める。gptelはバッファの表示されている部分のみを対象とできる
    - org-tree-to-indirect-buffer(C-c C-x b) でサブツリーを別バッファにする
    - C-c C-, からストラクチャーブロック(C comment)を作って org-narrow-to-block でバッファの表示範囲を狭める
      - narrowing は全体が見えなくなるのでorg-tree-to-indirect-bufferと組み合わせる

- コマンド
  - gptel-send :: カーソル位置より前のバッファもしくはリージョンを送信する
  - gptel :: 専用のチャットバッファを新規作成します
  - gptel-rewrite :: 選択した領域を書き換え、リファクタリング、または変更します
  - C-u gptel-send :: もしくは gptel-menu メニュー表示
  - gptel-add :: gptelのコンテキストに領域またはバッファをDired上でのマークされたファイルを追加/削除
  - gptel-add-file :: gptelのコンテキストにファイルを追加

  - gptel-org-set-topic :: コンテキストを指定した見出しに限定する
  - gptel-org-set-properties :: チャットの設定をプロパティとして設定する
...
(use-package gptel
  ...
  :commands
  (gptel gptel-send gptel-menu)

  :init
  (setq gptel-directives (my/load-my-eld "gptel.eld" 'system-prompt)) ; 設定タイミングの都合でデフォルト値が復元されることがあるので :init で設定
  (defun my-agent-browser-page-text (url)
    "URLをagent-browserで開き、ページ本文テキストを返す。"
    (with-temp-buffer
      (call-process "agent-browser" nil t nil "open" url)
      (erase-buffer)
      (call-process "agent-browser" nil t nil "get" "text" "body")
      (buffer-string)))

  :custom
  (gptel-default-mode 'org-mode)
  (gptel-model 'gpt-5.4-mini)   ; nil だと適切なものにフォールバックするけど、警告が出る
  (gptel-include-reasoning "*gptel reasoning buffer*") ; reasoning の内容を別バッファに表示する
  ;; (gptel-org-branching-context t)       ; gptel-org-set-topic で指定したほうが便利そう
  ;; (gptel-highlight-methods '(fringe face))

  ;; :hook
  ;; (gptel-mode-hook . gptel-highlight-mode)   ; org-modeのインデントと相性が悪そうなので一旦取りやめ
  ...
  (setq gptel-tools
        (list
         (gptel-make-tool
          :name "read_web_page"
          :function #'my-agent-browser-page-text
          :description "指定URLのWebページをagent-browserで開き、body要素のテキストを返す。"
          :args (list '(:name "url"
                              :type string
                              :description "読みたいWebページのURL"))
          :category "web"
          :confirm t
          :include t)))
...
** org-ai
*gptelを常用するので一旦 disabled t をセットした*

APIキーはデフォルトでauthinfoから読み込まれる。machine が api.openai.com で、 login が org-ai のものが参照される。
他のLLMバックエンドの設定はホスト個別の設定で行う。

- begin_ai, end_ai のストラクチャーブロックは C-c C-, で作成できる
- begin_ai, end_ai ブロック内で
...
- bagin_ai の直後に言語名(? 例えば、markdown)を書くとブロック内でシンタックスハイライトが効く

| org-ai-on-region               | C-c M-a r | 選択したテキストについて質問か指示を行う                           |
| org-ai-summarize               | C-c M-a s | 選択したテキストを要約する                                         |
| org-ai-refactor-code           | C-c M-a c | 選択したコードのリファクタリングを行う                             |
| org-ai-on-project              | C-c M-a p | 複数ファイルに対してプロンプトを実行する                           |
| org-ai-prompt                  | C-c M-a P | ユーザーにテキストの入力を促し、AIの応答を現在のバッファに表示する |
| org-ai-switch-chat-model       | C-c M-a m | モデルを変更                                                       |
| org-ai-open-account-usage-page | C-c M-a $ | openaiのusageページを開く                                          |
| org-ai-open-request-buffer     | C-c M-a ! | リクエストバッファを開く(デバッグ用)                               |
| org-ai-talk-input-toggle       | C-c M-a t | 音声入力を有効化                                                   |
| org-ai-talk-output-toggle      | C-c M-a T | 音声出力を有効化                                                   |
...
(use-package org-ai
  ...
  (org-ai-default-chat-model "gpt-5.4-mini")
  (org-ai-default-chat-system-prompt "You are a helpful assistant inside Emacs. Please answer in Japanese unless special instructions."))
#+end_src
...
(use-package yatex
  ...
  (setq tex-command "lualatex -synctex=1 -shell-escape")
  (setq YaTeX-kanji-code nil)
  (setq YaTeX-latex-message-code 'utf-8)
  (setq dvi2-command "zathura -P 1 -x \"emacsclient --no-wait +%{line} %{input}\"")
  (setq tex-pdfview-command "zathura -P 1 -x \"emacsclient --no-wait +%{line} %{input}\"")
  (setq dviprint-command-format "pdftops %t %s -level3 %f - | lpr")
  (setq dviprint-from-format "-f %b")
  (setq dviprint-to-format "-l %e")
  (setq YaTeX-user-completion-table (locate-user-emacs-file "yatex/yatexrc"))
  ;; (setq YaTeX-template-file (locate-user-emacs-file "yatex/templates/LaTeX-template.tex"))
  (setq YaTeX-singlecmd-suffix "{}")
  (setq YaTeX-fill-column 119))
...
;;
;;   :hook
;;   (prog-mode-hook . flymake-posframe-mode))
...
(use-package consult-ghq
  :straight (consult-ghq :type git :host github :repo "tomoya/consult-ghq"))
#+end_src
...
**** python-lsp-server
各プロジェクトごとに有効にするプラグインが異なるため .dir-locals-2.el に記述する

以下のサンプルを適宜コピーして使用する。
各プラグインの enabled は python-lsp-server のデフォルト値。(isortとblackは追加パッケージにて有効)

python-lsp-ruff は別途明示していない限り autopep8, mccabe, pycodestyle, pyflakes, yapf を無効にする。
そのため、以下のリストからは除外して転記したほうが良い。

- pydocstyleは開発が終了しているみたいなので無効にする
- rope_autoimportは有効にしていると安定しないので無効にする。折を見て再確認する

*デフォルトだと pylint が無効になっているので気をつける* 。

#+begin_src emacs-lisp :tangle no
((python-base-mode . ((eglot-workspace-configuration .
                         (:pylsp (:plugins (:autopep8 (:enabled t) ; disabling required to use yapf
                                            ...
                                            :jedi_type_definition (:enabled t)
                                            :mccabe (:enabled t)
                                            :pycodestyle (:enabled t)
                                            :pydocstyle (:enabled :json-false :conversion "google")
                                            :pyflakes (:enabled t)
                                            :pylint (:enabled :json-false :executable "pylint")
...
** GitHub copilot
[[id:74411c17-874d-45ed-a0a5-135411b9c677][Emacs with GitHub Copilot]]

利用前に M-x copilot-login を実行する。

設定項目として =copilot-network-proxy= が存在するがHTTP_PROXYは尊重されている模様。

プロジェクトごとにどの +言語+ モードに対してCopilotを有効・無効にするかを制御する必要があるため =copilot-mode= を =prog-mode= にフックはしない。
=.dir-locals.el= に以下を記載してモードごとに有効にする。
...
(use-package copilot
  ...
  :init
  (defun my/git-tracked-file-p (file)
    "Return non-nil if FILE is tracked in Git."
  ...
  (defun my/git-ignored-file-p (file)
    "Return non-nil if FILE is ignored in Git."
    (let ((default-directory (locate-dominating-file file ".git")))
      (and default-directory
        (eq 0
          (call-process "git" nil nil nil
  ...
  (defun my/copilot-disabled-p ()
    (and buffer-file-name
      ;; (my/git-tracked-file-p buffer-file-name)
      (my/git-ignored-file-p buffer-file-name)))

  :commands
  (copilot-mode copilot-login)

  :custom
  (copilot-disable-predicates
   `(my/copilot-disabled-p))
  (copilot-lsp-settings  '(:telemetry (:telemetryLevel "off")))

  :bind
  (:map copilot-completion-map
        ; tab は corfu で利用しているので、別のキーへ逃がす
        ;; ("M-<tab>" . copilot-accept-completion)
        ;; ("C-<tab>" . copilot-accept-completion)
        ;; ("C-<return>" . copilot-accept-completion)
        ("M-<return>" . copilot-accept-completion)
        ("C-M-<return>" . copilot-accept-completion-by-line)
        ("C-c C-<return>" . copilot-accept-completion-by-word)
        ("C-c C-]" . copilot-next-completion)
        ("C-c C-[" . copilot-previous-completion))
  ;; :hook
  ;; (prog-mode-hook . copilot-mode) ; 上述の通り prog-mode に copilot-mode はフックしない
)
#+end_src
...
気になっているもの
- [[https://github.com/dcluna/agent-shell-bookmark][dcluna/agent-shell-bookmark]]
- [[https://github.com/zackattackz/agent-shell-notifications][zackattackz/agent-shell-notifications]]
- [[https://github.com/nineluj/agent-review][nineluj/agent-review]]
...
(use-package agent-shell
  ...
  :preface
  (defun my/agent-shell-viewport-input-method-off (&rest _)
    (deactivate-input-method))

  :autoload agent-shell-make-environment-variables

  :custom
  ;; 全般設定 コメントアウトしているものはデフォルト値
  ;; (agent-shell-clipboard-image-handlers )         ; クリップボード画像をファイルに保存するハンドラ
  ;; (agent-shell-screenshot-command )               ; スクリーンショットを取得する際に使用するプログラム

  ;; (agent-shell-show-welcome-message t)                 ; 非 nil の場合、ウェルカムメッセージを表示する
  ;; (agent-shell-highlight-blocks nil)                   ; ソースブロックをハイライトするか否か https://github.com/xenodium/agent-shell/issues/119
  ;; (agent-shell-thought-process-expand-by-default nil)  ; 思考プロセスのセクションをデフォルトで展開するか否か
  ;; (agent-shell-tool-use-expand-by-default nil)         ; ツール使用セクションをデフォルトで展開するか否か
  ;; (agent-shell-user-message-expand-by-default nil)     ; ユーザーメッセージのセクションをデフォルトで展開するか否か
  (agent-shell-show-usage-at-turn-end t)                  ; エージェントターン終了時に使用情報を表示するか否か

  ;; (agent-shell-embed-file-size-limit 102400)           ; ContentBlock::Resource による埋め込みの最大ファイルサイズ（バイト）
  ;; (agent-shell-file-completion-enabled t)              ; 非 nil の場合、シェル起動時にファイル補完が自動的に有効になる
  ;; (agent-shell-text-file-capabilities t)               ; エージェントが読み書き可能なテキストファイル機能で初期化されるか否か
  (agent-shell-prefer-viewport-interaction t)             ; 非 nil の場合、‘agent-shell’ はシェル操作よりビューポート操作を優先する

  ;; エージェントごとの設定 は ホスト固有の設定ファイルで行う
  ...
  :config
  (advice-add 'agent-shell-viewport-view-mode
              :before
              #'my/agent-shell-viewport-input-method-off))

(use-package agent-shell-attention
  :straight (agent-shell-attention :type git :host github :repo "ultronozm/agent-shell-attention.el")
  :after agent-shell

  :bind
  ("<f6> A" . agent-shell-attention-dashboard)
  ...
  (setopt agent-shell-attention-indicator-location 'mode-line-misc-info)
...
(use-package agent-shell-manager
  ...
  :bind
  ("<f6> a" . agent-shell-manager-toggle))
#+end_src
...
以下をプロジェクトごとに .dir-locals.el もしくは .dir-locals-2.el への追記
- venvを.venvに作っていない場合 (venv-workon "venv名")
- poetry 利用プロジェクトで ruff-format 利用時に ruff-format-command の設定

#+begin_src emacs-lisp :tangle no
((python-base-mode . ((eval . (venv-workon "venv名"))
                      (customize-set-variable 'ruff-format-command "poetry run ruff")
                      ))
#+end_src
...
(use-package auto-virtualenvwrapper
  ...
  :hook
  (python-ts-mode-hook . auto-virtualenvwrapper-activate))
...
(use-package prettier-js
  ...
  :hook
  ((js-ts-mode-hook typescript-ts-mode-hook tsx-ts-mode-hook mhtml-mode-hook css-ts-mode-hook) . prettier-js-mode))
...
(use-package add-node-modules-path
  ...
  :hook
  ((js-ts-mode-hook typescript-ts-mode-hook tsx-ts-mode-hook) . add-node-modules-path))
#+end_src
...
(use-package org
  ...
  :preface
  (defun my/org-agenda-skip-not-ready ()
    "Skip entries whose SCHEDULED date has not arrived."
    (let ((scheduled (org-entry-get nil "SCHEDULED")))
      (when (or (null scheduled)
                (> (org-time-string-to-absolute scheduled)
                   (org-today)))
        (org-entry-end-position))))

  (defun my/org-agenda-skip-ready ()
    "Skip entries whose SCHEDULED date has already arrived."
    (let ((scheduled (org-entry-get nil "SCHEDULED"))
          (state (org-get-todo-state)))
      (unless
          (or
           ;; 未来に予定されているが state が SCHEDULED ではない
           (and scheduled
                (not (string= state "SCHEDULED"))
                (> (org-time-string-to-absolute scheduled)
                   (org-today)))
           ;; state は SCHEDULED なのに予定日がない
           (and (null scheduled)
  ...
  (org-directory "~/org")
  (org-agenda-files (my/load-my-eld "org.eld" 'agenda-files))
  ...
  (org-clock-mode-line-total 'current)      ;org-clock-clocked-in-displayで表示される時間を(累積でなく)現在の計測分に限る
  ...
  (with-eval-after-load 'org-agenda
    (add-to-list
     ...
     '("R" "Future scheduled tasks"
       tags "-dummy" ; 本当であれば todo "-SCHEDULED" としたい
       ((org-agenda-overriding-header "Future scheduled tasks")
        (org-agenda-skip-function
         #'my/org-agenda-skip-ready))))
    (add-to-list
     'org-agenda-custom-commands
     '("r" "Ready scheduled tasks"
       todo "SCHEDULED"
       ((org-agenda-overriding-header "Ready scheduled tasks")
        (org-agenda-skip-function
         #'my/org-agenda-skip-not-ready)))))
...
(use-package org-appear
  ...
  :custom
  (org-appear-autoemphasis t)      ; if non-nil and org-hide-emphasis-markers is on, toggle emphasis markers
  (org-appear-autolinks t)         ; if non-nil and org-link-descriptive is on, toggle links
  (org-appear-autosubmarkers nil)  ; if non-nil and org-pretty-entities is on, toggle subscripts and superscripts
  (org-appear-autoentities  t)     ; if non-nil and org-pretty-entities is on, toggle Org entitites
  (org-appear-autokeywords  t)     ; if non-nil and org-hidden-keywords is on, toggle keywords in org-hidden-keywords
  (org-appear-inside-latex  t)     ; if non-nil, toggle entities and sub/superscripts in LaTeX fragments
  (org-appear-delay  0.5)          ; seconds of delay before toggling
  (org-appear-trigger 'on-change)  ; when to toggle elements
...
(use-package org-journal
  ...
  (defun my/org-clock-in-to-journal ()
    "org-clock-in 時に journal に開始ログを追加"
    (when (org-clocking-p)
      (let* ((heading (org-get-heading t t t t))
        ...
        (with-current-buffer (find-file-noselect journal-file)
          ;; 見出しがない場合は作る
          (unless (org-find-exact-headline-in-buffer (format-time-string "%F (%a)"))
            (goto-char (point-max))
            (insert (format-time-string "\n* %F (%a)\n")))
          ;; journal セクションの末尾に移動
          (goto-char (org-find-exact-headline-in-buffer (format-time-string "%F (%a)") nil t))
          (org-end-of-subtree t t)
          ;; ログを追加
          (insert (format "** %s ▼ %s 開始\n" time heading))
          (save-buffer)))))

  (defun my/org-clock-out-to-journal ()
    "org-clock-out 時に journal に終了ログを追加"
    (let* ((heading (org-get-heading t t t t))
           (time (format-time-string "%H:%M"))
           (journal-file (org-journal--get-entry-path)))
      (with-current-buffer (find-file-noselect journal-file)
  ...
  (org-journal-file-header "#+TITLE: Daily Journal %Y-%m-%d (w:%V)\n#+FILETAGS: :journal:\n\n* Tasks\n\n* %F (%a)\n")
  ...
  :hook
  (org-clock-in-hook . my/org-clock-in-to-journal)
  (org-clock-out-hook . my/org-clock-out-to-journal))
#+end_src
...
- タイトルとファイル名の変更
  - #+TITLE: を変えて保存する
  - ファイル名を変更して =M-x org-roam-db-sync= を実行
...
(use-package org-roam
  ...
  (org-roam-capture-templates '(
    ("f" "Fleeting Notes" entry "* ${title} %U\n%i%?" :target (file+head "fleeting/note.org" "#+title: Fleeting Notes") :unnarrowed t :prepend t)
    ("p" "Permanent Notes" plain "%i%?\n" :target (file+head "${directory}/${slug}.org" ":PROPERTIES:\n:CREATED:  %<%Y-%m-%d_%H:%M:%S>\n:END:\n#+title: ${title}") :unnarrowed t)))

  (org-roam-capture-ref-templates '(
    ("r" "ref" plain "%?" :target (file+head "literature/web-articles/${slug}.org" "#+title: ${title}") :unnarrowed t)
    ("b" "books" plain "* まとめ\n* 書いてあったこと\n%?\n* 読んで思ったこと\n" :target (file+head "literature/books/${slug}-%i.org" ":PROPERTIES:\n:ISBN: %i\n:END:\n#+title: ${title}\n") :unnarrowed t)
...
(use-package lookup
  ...
  (lookup-search-agents (my/load-my-eld "lookup.eld" 'search-agents))
...
ミニバッファのプロンプトでは翻訳対象を書き直すことも可能。C-n, C-p で翻訳方向を切り替えることもできる。(可能な場合のみ)
翻訳結果はバッファでは対訳で表示されるが、キルリングには訳文のみが保存されている。
...
(use-package gt
  ...
  :config
  (setq gt-preset-translators `((google . ,(gt-translator
                                            :taker (gt-taker
                                                    :text 'paragraph
                                                    :langs '(ja en)
                                                    :prompt t
                                                    ;; :prompt 'buffer ;バッファの場合プロンプトでの文字列の変更が反映されない
                                                    :pick 'sentence)
...
(auth-source-pass-enable) ; password-store(pass) を使用する
...
my-dataディレクトリにある init-ホスト名.el をロードする。基本的にバージョン管理には含めない。

#+begin_src emacs-lisp
(setq init-host-el (locate-user-emacs-file (file-name-concat "my-data" (concat "init-" (car (split-string system-name "\\.")) ".el"))))
